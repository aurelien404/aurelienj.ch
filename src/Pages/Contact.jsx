...
function Contact() {
  ...
  const isValidUrl = (url) => {
    ...
    return /^(https?:\/\/)?(www\.)?[\w-]+\.(com|net|org|fr|ch|io|dev|app|info|be|pro)(\/[\w\-./?%&=]*)?$/.test(
      url.trim()
    );
  };
    ...
    <>
      <div className="w-full h-[120vh] md:h-[120vh] flex flex-col md:flex-row items-center md:items-start justify-center md:justify-between px-6 md:px-20 py-20">
        <div className="w-full md:w-3/5 h-full flex flex-col justify-center gap-6">
          ...
          <p
            className="text-lg text-zzbase font-normal w-full md:w-4/6"
          ...
          <div className="w-full md:w-4/6 grid gap-3 justify-items-center">
            ...
            {message && (
              <div
                className={`text-center sm:text-sm md:text-base ${
            ...
            <button
              className={`w-4/6 md:w-2/5 border-2 py-4 px-3 md:py-2 transform transition duration-200 hover:scale-105 active:scale-95 hover:border-zzlink focus:border-zzlink active:border-zzlink hover:text-zzlink focus:text-zzlink active:text-zzlink
                            ${
                              isLoading
              ...
              onClick={() => {
                plausible("contact_btn_send_clicked");
                handleSend();
              }}
              disabled={isLoading}
            >
