...
function Footer() {
  ...
  return (
    <footer
      id="quote"
      className="w-screen bg-zzcontrast border-t-2 border-zzbase"
    >
      <div className="py-30 w-full mx-auto flex flex-col items-center text-center border-b-2 border-zzbase">
        ...
        <p className="w-5/6 text-sm md:text-lg font-extralight">
          {t("footer.subtitle")}
        </p>
        <p className="text-[10px] md:text-xs font-extralight">
          "{t("footer.note")}
        </p>

        <div
          className={`w-auto mx-auto h-10  flex items-center text-center sm:text-sm md:text-base ${
        ...
        <div className="flex flex-col sm:flex-row items-center w-[90%] md:w-3/6 gap-4">
          <input
            ...
            className="flex justify-between w-full md:w-4/5 border-2 py-4 px-3 md:py-2 focus:border-zzlink focus:text-zzlink hover:scale-105 transition duration-200 active:scale-100 "
            type="email"
            placeholder={t("footer.mail")}
          />

          <button
            className={`w-full md:w-1/5 border-2 py-4 px-3 md:py-2 transform transition duration-200 hover:scale-105 active:scale-95
                            hover:border-zzlink focus:border-zzlink active:border-zzlink
                            hover:text-zzlink focus:text-zzlink active:text-zzlink
                            ${
                              isLoading
                                ? "animate-shake text-zzlink border-zzlink"
                                : ""
                            }`}
            onClick={handleSend}
            disabled={isLoading}
      ...
      <div className="w-full py-10 md:py-25 mx-auto flex flex-col items-center bg-zzlink text-zzcontrast">
        <a
          href="/"
          className="text-3xl md:text-5xl w-full text-center font-extrabold transition-all duration-300 ease-out"
        >
        ...
        <nav className="flex gap-6 text-sm md:text-lg mt-6 md:mt-10">
          {footerNav.map((link) => (
            <Link
              ...
              className={`link-zzstyle ${
                location.pathname === "{ link.to }" ? "link-zzstyle-active" : ""
              }`}
            >
              {t(link.labelKey)}
            </Link>
        ...
        <nav className="flex flex-row gap-4 text-xs md:text-sm font-extralight mt-2">
          <a href="/PP" className="link-style">
            {t("link.PP")}
          </a>
          <a href="/LN" className="link-style">
            {t("link.LN")}
          </a>
        </nav>
      </div>
